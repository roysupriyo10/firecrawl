...
defmodule Firecrawl do
  ...
  @support_ask_schema NimbleOptions.new!([
    context: [type: :any, doc: "Free-form metadata from the calling agent, stringified into the diagnostic prompt."],
    job_id: [type: :string, doc: "Optional Firecrawl job ID the failing call was associated with. Tools like debugJob, searchLogs, and getJob auto-default to this when set, so the agent doesn't need to pull it from the question."],
    question: [type: :string, required: true, doc: "What to diagnose. Describe the issue you're experiencing."],
    rationale: [type: :string, doc: "Recommended for AI callers. 1-2 sentences on what the end user is trying to accomplish."]
  ])

  @support_ask_key_mapping %{context: "context", job_id: "jobId", question: "question", rationale: "rationale"}
  ...
  Validated by `NimbleOptions`. Pass params as a keyword list with snake_case keys.
  See `@support_ask_schema` for the full schema.
  ...
  ## Returns
    ...
    * `{:error, exception}` on HTTP or validation failure
  """
  @spec support_ask(keyword(), keyword()) :: response()
  def support_ask(params \\ [], opts \\ []) do
    with {:ok, params} <- NimbleOptions.validate(params, @support_ask_schema) do
      Req.post(client(opts), url: "/support/ask", json: to_body(params, @support_ask_key_mapping))
    end
  end


  @doc """
  Bang variant of `support_ask`. Raises on error.
  """
  @spec support_ask!(keyword(), keyword()) :: Req.Response.t()
  def support_ask!(params \\ [], opts \\ []) do
    params = NimbleOptions.validate!(params, @support_ask_schema)
    Req.post!(client(opts), url: "/support/ask", json: to_body(params, @support_ask_key_mapping))
  end


  @support_docs_search_schema NimbleOptions.new!([
    question: [type: :string, required: true, doc: "The question to answer from Firecrawl's public documentation."]
  ])

  @support_docs_search_key_mapping %{question: "question"}
  ...
  Validated by `NimbleOptions`. Pass params as a keyword list with snake_case keys.
  See `@support_docs_search_schema` for the full schema.
  ...
  ## Returns
    ...
    * `{:error, exception}` on HTTP or validation failure
  """
  @spec support_docs_search(keyword(), keyword()) :: response()
  def support_docs_search(params \\ [], opts \\ []) do
    with {:ok, params} <- NimbleOptions.validate(params, @support_docs_search_schema) do
      Req.post(client(opts), url: "/support/docs-search", json: to_body(params, @support_docs_search_key_mapping))
    end
  end


  @doc """
  Bang variant of `support_docs_search`. Raises on error.
  """
  @spec support_docs_search!(keyword(), keyword()) :: Req.Response.t()
  def support_docs_search!(params \\ [], opts \\ []) do
    params = NimbleOptions.validate!(params, @support_docs_search_schema)
    Req.post!(client(opts), url: "/support/docs-search", json: to_body(params, @support_docs_search_key_mapping))
  end
