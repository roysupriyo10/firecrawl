...
export default class FirecrawlApp {
  ...
  > {
    ...
    let jsonData: any = {
      ...
      ...params,
      origin:
        typeof (params as any).origin === "string" &&
        (params as any).origin.includes("mcp")
          ? (params as any).origin
          : `js-sdk@${this.version}`,
    };
        ...
        scrapeOptions: {
          ...
          extract: {
            ...
            schema: zodSchemaToJsonSchema(
              jsonData.scrapeOptions.extract.schema,
            ),
          },
        },
      };
   ...
   */
  async getCreditUsageHistorical(
    byApiKey?: boolean,
  ): Promise<CreditUsageHistoricalResponseV1 | ErrorResponse> {
    const headers = this.prepareHeaders();
    try {
   ...
   */
  async getTokenUsageHistorical(
    byApiKey?: boolean,
  ): Promise<TokenUsageHistoricalResponseV1 | ErrorResponse> {
    const headers = this.prepareHeaders();
    try {
...
export class CrawlWatcher extends TypedEventTarget<CrawlWatcherEvents> {
  ...
  constructor(id: string, app: FirecrawlApp) {
    ...
    type Message =
      ErrorMessage | CatchupMessage | DoneMessage | DocumentMessage;
