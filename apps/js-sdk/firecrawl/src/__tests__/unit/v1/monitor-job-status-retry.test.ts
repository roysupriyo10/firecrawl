...
describe("monitorJobStatus retry logic", () => {
  ...
  test("should retry on socket hang up error", async () => {
    ...
    const successResponse = {
      ...
      data: {
        status: "completed",
        data: [{ url: "test.com", markdown: "test" }],
      },
    };
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Network error during job status check (attempt 1/3): socket hang up",
      ),
    );
  });
  ...
  test("should fail after max retries exceeded", async () => {
    ...
    await expect(app.monitorJobStatus("test-id", {}, 1)).rejects.toThrow(
      "socket hang up",
    );
