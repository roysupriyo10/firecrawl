...
describe("FirecrawlApp E2E Tests", () => {
  ...
  test.concurrent(
    ...
    async () => {
      if (API_URL.includes("api.firecrawl.dev")) {
        const invalidApp = new FirecrawlApp({
          apiKey: "invalid_api_key",
          apiUrl: API_URL,
        });
        await expect(
          invalidApp.scrapeUrl("https://firecrawl-test-site.vercel.app"),
        ).rejects.toThrow(
          "Unexpected error occurred while trying to scrape URL. Status code: 401",
        );
      } else {
        const invalidApp = new FirecrawlApp({
          apiKey: "invalid_api_key",
  ...
  test.concurrent(
    "should throw error for unsupported URL on scrape",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const unsupportedUrl = "https://facebook.com/fake-test";
      await expect(app.scrapeUrl(unsupportedUrl)).rejects.toThrow(
  ...
  test.concurrent(
    "should return successful response with valid API key and options",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = await app.scrapeUrl(
        "https://firecrawl-test-site.vercel.app",
  ...
  test.concurrent(
    "should return successful response with valid API key and screenshot fullPage",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = await app.scrapeUrl(
        "https://firecrawl-test-site.vercel.app",
  ...
  test.concurrent(
    "should return successful response for valid scrape with PDF file and parsePDF true",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = await app.scrapeUrl(
        "https://arxiv.org/pdf/astro-ph/9301001.pdf",
  ...
  test.concurrent(
    "should return successful response for valid scrape with PDF file and parsePDF false",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = await app.scrapeUrl(
        "https://arxiv.org/pdf/astro-ph/9301001.pdf",
  ...
  test.concurrent(
    ...
    async () => {
      if (API_URL.includes("api.firecrawl.dev")) {
        ...
        await expect(
          invalidApp.crawlUrl("https://firecrawl-test-site.vercel.app"),
  ...
  test.concurrent(
    "should return successful response for crawl with options and wait for completion",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = (await app.crawlUrl(
        "https://firecrawl-test-site.vercel.app",
  ...
  test.concurrent("should handle idempotency key for crawl", async () => {
    ...
    const response = (await app.asyncCrawlUrl(
      "https://firecrawl-test-site.vercel.app",
      {},
  ...
  test.concurrent(
    ...
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = (await app.asyncCrawlUrl("https://firecrawl.dev", {
        limit: 20,
        scrapeOptions: {
          formats: ["markdown", "html", "rawHtml", "screenshot", "links"],
        },
      } as CrawlParams)) as CrawlResponse;
      expect(response).not.toBeNull();
      expect(response.id).toBeDefined();
      ...
      while (
        (statusResponse as any).status === "scraping" &&
        checks < maxChecks
      ) {
        ...
        if (statusResponse.success === true) {
          ...
          expect(statusResponse.expiresAt.getTime()).toBeGreaterThan(
            Date.now(),
          );
  ...
  test.concurrent("should throw error for invalid API key on map", async () => {
    if (API_URL.includes("api.firecrawl.dev")) {
      ...
      await expect(
        invalidApp.mapUrl("https://firecrawl-test-site.vercel.app"),
      ).rejects.toThrow("Request failed with status code 401");
    } else {
      const invalidApp = new FirecrawlApp({
        apiKey: "invalid_api_key",
  ...
  test.concurrent(
    "should return successful response for valid map",
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = (await app.mapUrl(
        "https://firecrawl-test-site.vercel.app",
      )) as MapResponse;
      expect(response).not.toBeNull();
      ...
      const filteredLinks = response.links?.filter((link: string) =>
        link.includes("firecrawl-test-site.vercel.app"),
      );
  ...
  test("should handle invalid API key for search", async () => {
    ...
    await expect(app.search("test query")).rejects.toThrow(
