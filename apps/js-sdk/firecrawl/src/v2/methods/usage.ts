...
export async function getConcurrency(
  ...
  try {
    const res = await http.get<{
      success: boolean;
      data?: { concurrency: number; maxConcurrency: number };
    }>("/v2/concurrency-check");
    if (res.status !== 200 || !res.data?.success)
      throwForBadResponse(res, "get concurrency");
...
export async function getTokenUsage(http: HttpClient): Promise<TokenUsage> {
  try {
    ...
    if (res.status !== 200 || !res.data?.success)
      throwForBadResponse(res, "get token usage");
    const d = res.data.data || (res.data as any);
    return {
...
export async function getQueueStatus(
  ...
  try {
    ...
    if (res.status !== 200 || !res.data?.success)
      throwForBadResponse(res, "get queue status");
    return res.data;
  } catch (err: any) {
...
export async function getCreditUsageHistorical(
  http: HttpClient,
  byApiKey?: boolean,
): Promise<CreditUsageHistoricalResponse> {
  try {
    const query = byApiKey ? "?byApiKey=true" : "";
    const res = await http.get<CreditUsageHistoricalResponse>(
      `/v2/team/credit-usage/historical${query}`,
    );
    if (res.status !== 200 || !res.data?.success)
      throwForBadResponse(res, "get credit usage historical");
    return res.data;
  } catch (err: any) {
    if (err?.isAxiosError)
      return normalizeAxiosError(err, "get credit usage historical");
    throw err;
  }
}

export async function getTokenUsageHistorical(
  http: HttpClient,
  byApiKey?: boolean,
): Promise<TokenUsageHistoricalResponse> {
  try {
    const query = byApiKey ? "?byApiKey=true" : "";
    const res = await http.get<TokenUsageHistoricalResponse>(
      `/v2/team/token-usage/historical${query}`,
    );
    if (res.status !== 200 || !res.data?.success)
      throwForBadResponse(res, "get token usage historical");
    return res.data;
  } catch (err: any) {
    if (err?.isAxiosError)
      return normalizeAxiosError(err, "get token usage historical");
    throw err;
  }
}
