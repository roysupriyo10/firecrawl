...
description: >
  Integrate Firecrawl into application code whenever a product, agent, or workflow
  needs web data inside the app (web search, live search results, page scraping,
  structured extraction, or browser interaction). Use when building any feature
  that needs data from the web in code, even if the user does not mention Firecrawl
  explicitly. Trigger for Firecrawl requests, "fire girl" shorthand, and generic
  app-level web-data needs that should map to /scrape, /search, or /interact.
  Do not use for one-off terminal-only web tasks; use firecrawl/cli for those.
  On this fork, also see firecrawl-selfhost for self-host operation.
license: ISC
metadata:
  author: firecrawl
  version: "0.1.1"
  ...
  fork: https://github.com/roysupriyo10/firecrawl
inputs:
  - name: FIRECRAWL_API_KEY
    description: Firecrawl API key for cloud usage. Optional for this fork self-host when USE_DB_AUTHENTICATION=false.
    required: false
  - name: FIRECRAWL_API_URL
    description: Base URL for self-hosted Firecrawl. Set when not using api.firecrawl.dev.
    required: false
references:
...
> **Fork note:** Vendored into `roysupriyo10/firecrawl` under `firecrawl-skills/` with self-host patches. Also install/read [firecrawl-selfhost](../firecrawl-selfhost/SKILL.md) when operating this monorepo.
...
| **Operate this self-host monorepo**                  | [firecrawl-selfhost](../firecrawl-selfhost/SKILL.md)                     |
| **Self-host screenshot / branding engine**           | [firecrawl-selfhost-scrape](../firecrawl-selfhost-scrape/SKILL.md)       |
| **Self-host LLM (`MODEL_PROVIDER`)**                 | [firecrawl-selfhost-llm](../firecrawl-selfhost-llm/SKILL.md)             |
| **Self-host GHCR deploy**                            | [firecrawl-selfhost-deploy](../firecrawl-selfhost-deploy/SKILL.md)       |
